 ...
 */
import { requestImageDataUrl } from './images.js';
...
export const MAX_SSE_DATA_LINES = 4096;
...
function assertSupportedImageRole(message) {
  if (message.role !== 'user' && contentHasImage(message.content)) {
    throw new WireError(
      `Grok request cannot represent image content in a ${message.role} message`,
      'UNSUPPORTED_CONTENT',
    );
  }
}

/** Flatten one content-block list to plain text without ever substituting images. */
function flattenText(blocks) {
  const parts = [];
  for (const block of blocks ?? []) {
    if (block.type === 'text' && block.text.length > 0) parts.push(block.text);
    else if (block.type === 'tool-result') {
      const nested = flattenText(block.content);
...
function responsesContent(blocks, prepared) {
  const content = [];
  for (const block of blocks ?? []) {
    if (block.type === 'text' && block.text.length > 0) {
      ...
      content.push({
        type: 'input_image',
        image_url: requestImageDataUrl(prepared, block.attachment),
        detail: 'auto',
      });
    } else if (block.type === 'tool-result') {
      content.push(...responsesContent(block.content, prepared));
    }
  }
  return content;
}

function toolResponsesOutput(block, prepared) {
  if (!contentHasImage(block.content)) return toolOutputText(block);
  const content = responsesContent(block.content, prepared);
  if (!content.some((part) => part.type === 'input_text')) {
    content.unshift({ type: 'input_text', text: '(no output)' });
  }
  if (block.isError === true) {
    const firstText = content.find((part) => part.type === 'input_text');
    firstText.text = `[tool error] ${firstText.text}`;
  }
  return content;
}

function chatContent(blocks, prepared) {
  const content = [];
  for (const block of blocks ?? []) {
    if (block.type === 'text' && block.text.length > 0) {
      ...
      content.push({
        type: 'image_url',
        image_url: { url: requestImageDataUrl(prepared, block.attachment) },
      });
    } else if (block.type === 'tool-result') {
      content.push(...chatContent(block.content, prepared));
    }
  }
  return content;
}

function toolChatOutput(block, prepared) {
  if (!contentHasImage(block.content)) return toolOutputText(block);
  const content = chatContent(block.content, prepared);
  if (!content.some((part) => part.type === 'text')) {
    content.unshift({ type: 'text', text: '(no output)' });
  }
  if (block.isError === true) {
...
export function serializeResponsesRequest(options, defaults, preparedImages) {
  const input = [];
  for (const message of options.messages) {
    assertSupportedImageRole(message);
    ...
    if (regular.length > 0 || toolResults.length === 0) {
      const hasImages = contentHasImage(regular);
      const content = hasImages
        ? responsesContent(regular, preparedImages)
        : (() => {
    ...
    for (const block of toolResults) {
      input.push({
        ...
        output: toolResponsesOutput(block, preparedImages),
      });
    }
  }
  const body = {
...
export async function* translateResponses(events) {
  ...
  if (!finished) throw new WireError('Grok stream closed before a terminal response event', 'STREAM_CLOSED');
}
...
export function serializeChatRequest(options, defaults, preparedImages) {
  const messages = [];
  ...
  for (const message of options.messages) {
    ...
    if (regular.length > 0 || toolResults.length === 0) {
      const hasImages = contentHasImage(regular);
      const content = hasImages ? chatContent(regular, preparedImages) : flattenText(regular);
      if ((Array.isArray(content) && content.length > 0) || (typeof content === 'string' && content.length > 0)) {
        messages.push({ role: message.role === 'system' ? 'system' : 'user', content });
      }
    }
    for (const block of toolResults) {
      messages.push({
        ...
        content: toolChatOutput(block, preparedImages),
      });
    }
  }
  const body = {
...
export async function* translateChat(events) {
  ...
  for await (const payload of events) {
    ...
    try {
      ...
      throw new WireError('Grok chat stream sent malformed JSON', 'MALFORMED_RESPONSE', { cause: error });
    }
  ...
  if (finishReason === undefined) {
    throw new WireError('Grok chat stream closed without a finish reason', 'STREAM_CLOSED');
  }
 ...
 * @param signal - aborts and actively cancels the owned stream reader.
 */
export async function* parseSse(body, mode, onActivity, signal) {
  ...
  const flush = function* () {
    ...
    try {
      ...
      throw new WireError('Grok stream sent a malformed SSE payload', 'MALFORMED_RESPONSE', { cause: error });
    }
  ...
  try {
    for (;;) {
      ...
      if (signal?.aborted) {
        throw signal.reason ?? new WireError('Grok stream aborted', 'ABORTED');
      }
      if (done) break;
      onActivity?.();
      try {
        ...
        throw new WireError('Grok stream sent malformed UTF-8', 'MALFORMED_RESPONSE', { cause: error });
      }
      let newline;
      while ((newline = buffer.indexOf('\n')) !== -1) {
        ...
        if (Buffer.byteLength(line, 'utf8') > MAX_SSE_LINE_BYTES) {
          throw new WireError('Grok stream exceeded the SSE line limit', 'SSE_LIMIT');
        }
        if (line.length === 0) {
          yield* flush();
          ...
          eventBytes += Buffer.byteLength(data, 'utf8') + (dataLines.length > 0 ? 1 : 0);
          if (eventBytes > MAX_SSE_EVENT_BYTES) {
            throw new WireError('Grok stream exceeded the SSE event limit', 'SSE_LIMIT');
          }
          if (dataLines.length >= MAX_SSE_DATA_LINES) {
            throw new WireError('Grok stream exceeded the SSE data-line limit', 'SSE_LIMIT');
          }
          dataLines.push(data);
        }
