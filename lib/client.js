window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function normalizeDirectoryResponse(response) {
			if (!response || response.result?.ok !== true || !Array.isArray(response.result.value?.groups)) {
				throw fixedError("directory_unavailable");
			}
			const matching = response.result.value.groups.filter((group) => group?.id === PROVIDER);
		...
		function apply(ctx) {
			...
			const directoryLoader = createDirectoryLoader({
				read: async () => normalizeDirectoryResponse(await api.llm.models({})),
				loading: () => {
					if (lifecycleDisposed) return;
