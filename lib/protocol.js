...
const INPUT_MODALITY_ORDER = Object.freeze(['text', 'image']);
const INPUT_MODALITIES = new Set(INPUT_MODALITY_ORDER);
const GROK_BUILD_IMAGE_COMPAT_INPUT_MODALITIES = Object.freeze(['text', 'image']);
export const GROK_BUILD_IMAGE_COMPAT_PROVENANCE =
  `${LIVE_CATALOG_CAPABILITY_PROVENANCE}:grok-build-image-compat-v1`;
...
export function buildProtocolHeaders(options = {}) {
  ...
  if (operation === 'catalog' && model !== undefined) {
    throw new TypeError('catalog requests must not carry a model override');
  }
  const safeModel = operation === 'inference' ? assertSafeModelId(model) : undefined;
...
export function normalizeEntitledModel(entry) {
  ...
  const rawModalities = consistentAlias(
    ...
    (value) => {
      ...
      if (unique.size !== value.length) throw new TypeError('catalog input modalities contain duplicates');
      if ([...unique].some((item) => !INPUT_MODALITIES.has(item))) {
        throw new TypeError('catalog input modalities contain an unsupported value');
      }
      return INPUT_MODALITY_ORDER.filter((item) => unique.has(item));
    },
  );
  ...
  const apiBackend = normalizedBackend;
  ...
  if (apiBackend === undefined || apiBackend.startsWith('unsupported:')) return undefined;
  // The authenticated live catalog remains authoritative whenever it declares
  // modalities. Older catalog shapes omit the field even though the pinned
  // Grok Build protocol sends structured images through both supported
  // backends without a model-version allowlist. Apply that versionless
  // provider/backend compatibility fact only to omission; explicit text-only
  // is never widened.
  const inputModalities = rawModalities === undefined
    ? GROK_BUILD_IMAGE_COMPAT_INPUT_MODALITIES
    : Object.freeze(rawModalities);
  if (!inputModalities.includes('text')) return undefined;
  ...
  const capabilityProvenance = rawModalities === undefined
    ? GROK_BUILD_IMAGE_COMPAT_PROVENANCE
    : `${LIVE_CATALOG_CAPABILITY_PROVENANCE}:explicit-modalities`;
  return Object.freeze({
    id,
    name,
    ...(description === undefined ? {} : { description }),
...
/** Resolve one exact model from a normalized live account catalog. */
export function entitledModelFromCatalog(document, model) {
  const safeModel = assertSafeModelId(model);
  return entitledModelsFromCatalog(document).find((entry) => entry.id === safeModel);
}
