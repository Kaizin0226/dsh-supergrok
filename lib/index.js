...
import { GrokAdapter, LIVE_CANARY_ENABLED } from './adapter.js';
import {
...
import { createPinnedTransport, postFormJson, resolveProxyUrl } from './net.js';
...
const NS = 'llm-grok-oauth';

export const Config = z.object({
  proxyUrl: z.string().description('必填：本地 HTTP 代理地址；修改后重启此插件，无直连回退'),
  ...
  maxRequestBodyBytes: z.number().step(1).min(1).description('本地最终请求字节预算（不是官方上限）').default(40000000),
});
...
export function resolveOptions(config = {}) {
  ...
  // Older profiles may still carry model/reasoningEffort keys. They are
  // intentionally ignored: selection and defaults come only from the current
  // authenticated account catalog and the DSH session request.
  ...
  return Object.freeze({
    proxyUrl: resolveProxyUrl(config.proxyUrl),
    ...
    maxRequestBodyBytes: positive(config.maxRequestBodyBytes, 40000000, 'maxRequestBodyBytes'),
  });
}
...
export function apply(ctx, config) {
  let current = () => config;
  const options = () => resolveOptions({ ...config, ...(current() ?? {}) });
  const initialOptions = options();
  const transport = createPinnedTransport({ proxyUrl: initialOptions.proxyUrl, currentProxyUrl: () => options().proxyUrl });
  ctx.effect(() => () => transport.close(), 'llm-grok-oauth: dispose owned proxy dispatcher');
  ...
  const oauth = new GrokOAuth(store, ctx.logger, publishStatus, {
    postForm: (pathname, form, timeoutMs) => postFormJson(pathname, form, timeoutMs, transport),
  });
  const usage = new UsageService(oauth, { fetchImpl: transport.fetch });
  ctx.effect(() => () => usage.clear(), 'llm-grok-oauth: dispose usage cache');
  const adapter = new GrokAdapter(options, oauth, ctx.logger, {
    fetch: transport.fetch,
    resolveAttachments: () => ctx.get('attachments'),
    beforeModelRequest: request => ctx.serial('grok-oauth/before-model-request', request),
  });

  ctx.provide('grokRequestBoundary', Object.freeze({ protocolVersion: 1, everyModelPost: true }));
  ...
  if (process.env.DSH_SUPERGROK_ACCEPTANCE !== '1' && !LIVE_CANARY_ENABLED) {
    ctx.effect?.(() => {
      let running = false;
      let disposed = false;
      const tick = async () => {
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    register('/api/llm-grok-oauth/usage', handler('GET', false, async (req) => ({
      usage: await usage.get(req.headers['x-dsh-usage-refresh'] === '1'),
    })), 'llm-grok-oauth: usage route');
